...
if(!$driver_found){
    ...
    lives_ok(sub { $bag = Catmandu::Store::DBI->new(data_source => "dbi:SQLite:dbname=$file")->bag()},"bag created");

    my $record = { _id => "test",title => "my little pony",author => "no idea" };
    ...
    #bag get
    my $new_record;
    lives_ok(sub { $new_record = $bag->get($record->{_id}); },"bag get successfull");
